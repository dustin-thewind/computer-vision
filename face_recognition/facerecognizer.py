...
class FaceRecognizer:
    def __init__(self, recognizer, trained=False, labels=None):
        # store the face recognizer, whether or not the recognizer
        # has already been trained, and the list of face name labels
    ...
    def setConfidenceThreshold(self, confidenceThreshold):
        # set the confideence threshold for the classifier
        if imutils.is_cv2():
    ...
    def predict(self, face):
        ...
        # if the prediction is '-1' then the confidence is greater
        # than the threshold, implying the face cannot be recognized
        if prediction == -1:
            return ("Unknown", 0)

        # return the tuple of the face label, and the confidence
    ...
    def save(self, basePath):
        ...
        # check to see if output classifier file exists prior to writing
        if not os.path.exists(basePath + "/classifier.model"):
            f = open(basePath + "/classifier.model", "w")
            f.close()

        # handle if we're writing the ocv 2.4 recognizer to file
        if imutils.is_cv2():
            self.recognizer.save(basePath + "/classifier.model")

        # otherwise, we're writing the OCV3 recognizer to file
        else:
    ...
    @staticmethod
    def load(basePath):
        # load the face recognition instance and construct the OCV face recognizer
        fri = pickle.loads(open(basePath + "/fr.cpickle", "rb").read())

        # handle if we are buidling an OCV 2.4 face recognizer
        if imutils.is_cv2():
            recognizer = cv2.createLBPHFaceRecognizer()
            recognizer.load(basePath + "/classifier.model")

        # otherwise we are buidling an OCV3 face recognizer
        else:
