...
ap.add_argument("-c", "--classifier", required=True, help = "path to the output classifier")
ap.add_argument("-n", "--sample-size", type=int, default=100, help = "max sample size for each face")
args = vars(ap.parse_args())

# init the face recognizer and the list of labels
if imutils.is_cv2():
...
for (i, path) in enumerate(glob.glob(args["selfies"] + "/*.txt")):
    ...
    # load the faces file, sample it, and init the list of faces
...
# update the face recognizer to inlcude the face name labels, then write the model to file
fr.setLabels(labels)
