...
class BlockBinaryPixelSum:
    def __init__(self, targetSize=(30, 15), blockSizes=((5, 5),)):
        # store the target size of the image to be described
        # along with the set of block sizes
        self.targetSize = targetSize
        self.blockSizes = blockSizes

    def describe(self, image):
        # resize the image to target size and init the feature vector
        ...
        for (blockW, blockH) in self.blockSizes:
            ...
            for y in range(0, image.shape[0], blockH):
                for x in range(0, image.shape[1], blockW):
                    # extract the current ROI, count total number of non-zero pixels
                    # in the ROI, normalizing by the total size of the block
