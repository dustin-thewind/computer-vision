# USAGE
# python visualize_centers.py --dataset images --features-db /output/features.h5py --codebook /output/vocab.cpickle \
# --output output/vw_vis

# import packages
from __future__ import print_function
from sklearn.metrics import pairwise
from cbir_test.cbir import ResultsMontage
import numpy as np
import progressbar
import argparse
import pickle
...
# load the codebook and open the features db
vocab = pickle.loads(open(args["codebook"], "rb").read())
featuresDB = h5py.File(args["features_db"], mode="r")
print("[INFO] starting distance computations...")

# init the visualizations dictionary and init the progress bar
...
for (i, imageID) in enumerate(featuresDB["image_ids"]):
    ...
    for (kp, features) in zip(kps, descs):
        ...
        # meaning that we'll have on distance value for each cluster
        ...
        for j in np.arange(0, len(vocab)):
            ...
            # the best 16, then update the visualizations dict
