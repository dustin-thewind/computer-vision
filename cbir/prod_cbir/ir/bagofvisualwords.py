...
class BagOfVisualWords:
    def __init__(self, codebook, sparse=True):
        # store the codebook used to compute the BOVW representation
        # for each image along with the flag used to control whether
        # sparse or dense histograms are constructed
        self.codebook = codebook
        self.sparse = sparse

    def describe(self, features):
        # compute the euclidean distance between the features and cluster centers
        # grab the indexes of the smallest distances for each cluster
        # and construct a BOVW representation
        ...
        # returnt he histogram of visual word counts
        return hist
