...
class SpatialVerifier:
    def __init__(self, featuresDBPath, idf, vocab, ratio=0.75, minMatches=10, reprojThresh=4.0):
        ...
        # feature matching, min  number of matches required to perform spatial
        # verification, and the reprojection threshold for computing inliers
        self.idf = idf
        self.vocab = vocab
        self.ratio = ratio
    ...
    def rerank(self, queryKps, queryDescs, searchResult, numResults=10):
        # start the search timer and init the re-ranked results dict
        startTime = datetime.datetime.now()
        reranked = {}

        # grab the image indexes from the initial search results and sort them in
        # ascending order so the feature indexes can be grabbed from HDFS
        resultIdxs = np.array([r[-1] for r in searchResult.results])
        resultIdxs.sort()

        # loop over the starting and ending indexes into the features dataset
        # for each image
        for (i, (start, end)) in zip(resultIdxs, self.featuresDB["index"][resultIdxs, ...]):
            ...
            # matched keypoints into the BOVW
            bovwIdxs = self.match(queryKps, queryDescs.astype("float32"), kps,
                descs.astype("float32"))

            # provided that at least some kp's were matched, the final score
            # for the spatial verification is the sum of the idf values for
            # the inlier words
            if bovwIdxs is not None:
                score = self.idf[bovwIdxs].sum()
                reranked[i] = score

            # if no spatially verified matches were found, return the initial match
            # result object
            if len(reranked) == 0:
                return searchResult
            ...
            # loop over the intial search results
            for (score, imageID, imageIdx) in searchResult.results:
                # only add the intial result to the list of results if the image
                # has NOT been spatially verified
    ...
    def match(self, kpsA, featuresA, kpsB, featuresB):
        # compute the raw matches and init the list of actual matches
        ...
        if len(matches) >= self.minMatches:
            ...
            # grab the inlier points where status > 0, then quantize
        ...
        # return the list of inlier idxs
        return inlierIdxs

    def finish(self):
        #close the features DB
