...
class FeatureIndexer(BaseIndexer):
    def __init__(self, dbPath, estNumImages=500, maxBufferSize=50000, dbResizeFactor=2,
        ...
        # open the hdf5 DB for writing and init the datasets within the group
        ...
        # init the image ID's buffer, index buffer, and
        # the keypoints + features buffer
        self.imageIDBuffer = []
        self.imageDimsBuffer = []
        self.indexBuffer = []
        self.featuresBuffer = None

        # init the total number of features in the buffer along with the
        # indexes dictionary
    ...
    def _createDatasets(self):
        # compute the average numnber of features extracted from the initial buffer
        # and use this number to determine the approx number of features
        ...
        # handle h5py data for python 2.7
    ...
    def finish(self):
        # if the databases have not been init'd then
        # the original buffers were never filled up
        if None in (self.imageIDDB, self.indexDB, self.featuresDB):
            self._debug("minimum init buffer not reached", msgType="[WARN]")
            self._createDatasets()

        # write the unempty buffers to file
