...
class ObjectDetector:
    ...
    def detect(self, image, winDim, winStep=4, pyramidScale=1.5, minProb=0.7):
        # init the list of bounding boxes and associated probabilities
        boxes = []
        probs = []
        ...
        for layer in helpers.pyramid(image, scale=pyramidScale, minSize=winDim):
            ...
            # loop over the sliding windows for the current pyrmaid layer
            for (x, y, window) in helpers.sliding_window(layer, winStep, winDim):
                ...
                if winH == winDim[1] and winW == winDim[0]:
                    ...
                    # not this window contains an object we're interested in
                    features = self.desc.describe(window).reshape(1, -1)
                    prob = self.model.predict_proba(features)[0][1]

                    # check to see if the calssfier has found an object with
                    # sufficient probability
                    if prob > minProb:
                        # compute the x,y coords of the boudning box using
                        # the current scale of the image pyramid
        ...
        # return a tuple of the boudning boxes and probs
        return (boxes, probs)
