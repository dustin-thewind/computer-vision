...
from obj_detect.object_detection import helpers
from obj_detect.descriptors import HOG
from obj_detect.utils import dataset
...
# init the HOG descriptor along with the list of data and labels
...
for (i, trnPath) in enumerate(trnPaths):
    ...
    # load the annotation file associated with the image and extract the boudning box
...
for i in np.arange(0, conf["num_distraction_images"]):
    ...
    for patch in patches:
        # extact the features from the patch, then update the data and label list
